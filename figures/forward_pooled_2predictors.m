...
tmp = vertcat(d.CC{:});
CC = reshape(tmp,[nSlices,nSub,2,nChan,nLambda]);
model = d.model;
...
model = reshape(model,[nLags,nFeatures,nSub,nLambda]);
...
trf = permute(model(:,:,:,iMaxCC),[1,3,2]);
